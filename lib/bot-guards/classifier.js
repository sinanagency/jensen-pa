// Intent classifier. Pure-ish — one HTTPS call to Anthropic, no DB, no global state.
//
// Returns a typed intent from config.intentEnum + a confidence + a one-line
// reason. Fail-open: errors return {intent: <first enum value>, confidence: "low"}.
// This is by design — the bot's worker should treat "low" confidence as
// "open_conversation" (or its equivalent fallback intent).
...
export async function classifyIntent(command, history, config, opts = {}) {
    ...
    try {
        ...
        const conf = ["high", "medium", "low"].includes(input.confidence) ? input.confidence : "medium";
        return {
            intent: input.intent,
            confidence: conf,
