        ...
        for c in json.load(open(shard)):
            ...
            for node in mapping.values():
                ...
                if len(content.split()) < MIN_WORDS:
                    continue
                # Reject paste-back contamination: em-dashes and en-dashes are a
                # strong signal that text was generated by GPT and pasted back
                # for refinement. Jensen does not type em-dashes natively in
                # WhatsApp/email. Filter for voice modelling.
                if "—" in content or "–" in content:
                    continue
                # Reject very-long messages: Jensen's own writing is short
                # (median 19 words). Anything > 400 words in a single user
                # turn is almost certainly a draft pasted in for him to ask
                # ChatGPT to polish, not his actual voice.
...
def render(msgs, stats, samples):
    ...
    L(f"- Paste-back filter: messages containing em-dashes or longer than 400 words were excluded as likely GPT-pasted drafts, not Jensen's native composition. The corpus below is his actual voice.")
    L("")
