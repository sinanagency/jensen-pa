// SPEC 001 — One-shot fire of the OpenAI-export onboarding bubbles to Jensen.
// Idempotent (kv flag `onboarding_bubbles_sent_at`). Requires ADMIN_SECRET
// header. Each bubble routes through sendTextAndLog (Law 2 chokepoint).
//
// Taona triggers via:
//   curl -X POST https://jensen-pa.vercel.app/api/_/send-onboarding-bubbles \
...
import {
  ONBOARDING_BUBBLES,
  PAUSE_MS_BETWEEN_BUBBLES,
} from "@/specs/001-export-mining/messages";

export const runtime = "nodejs";
export const maxDuration = 60;

const FLAG_KEY = "onboarding_bubbles_sent_at";
...
export async function POST(req: NextRequest) {
  ...
  if (dry) {
    return NextResponse.json({
      ...
      dry: true,
      to,
      bubble_count: ONBOARDING_BUBBLES.length,
      pause_ms: PAUSE_MS_BETWEEN_BUBBLES,
      previews: ONBOARDING_BUBBLES.map((b) => b.split("\n")[0].slice(0, 80)),
    });
  }

  const startedAt = Date.now();
  const results: { idx: number; ok: boolean }[] = [];
  for (let i = 0; i < ONBOARDING_BUBBLES.length; i++) {
    const body = ONBOARDING_BUBBLES[i];
    const r = await sendTextAndLog(to, body, { force: true, party: "jensen" });
    results.push({ idx: i, ok: r.ok });
    if (i < ONBOARDING_BUBBLES.length - 1) {
      await sleep(PAUSE_MS_BETWEEN_BUBBLES);
    }
  }
